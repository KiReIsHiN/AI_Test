...
if st.button("🚀 Создать сервер, установить окружение и запустить"):
    with st.spinner("Запрос к RunPod..."):
        ...
        if [ ! -d "/workspace/venv" ]; then
            ...
            cd MOSS-TTS && /workspace/venv/bin/pip install -e ".[torch-runtime]" && cd ..;
        fi &&
        wget -qO backend_server.py https://raw.githubusercontent.com/ВАШ_GITHUB/ВАШ_РЕПОЗИТОРИЙ/main/backend_server.py &&
